...
Als Ausgangsstoff ergibt sich somit für die Natronlauge $\nu(NaOH)= -1$. Zusammengeführt werden diese Zusammenhänge in Gl. \ref{gl:3} mit Hilfe der Stoffmengenänderungsgeschwindigkeit $R$.
\begin{flalign}
\label{gl:3}
...
Um über die Reaktionsgeschwindigkeit den Stoßfaktor $k$ bestimmen zu können wird diese in Zusammenhang mittels Potenzansatz in Gl. \ref{gl:4} geführt.
In Abb.\ref{dia:lnr/lnc} sind die lineare Verläufe der Messwerte unter Einbezug von Gl. \ref{gl:4} für die untersuchten Temperaturen aufgetragen.
...
\begin{figure}[h!]
	\begin{center}
		...
		\end{tikzpicture}
		%	}Ventilkennlinie
		\caption{Logarithmus der Reaktionsgeschwindigkeit über dem Logarithmus der Konzentration}
		\label{dia:lnr/lnc}
	\end{center}
\end{figure}
\FloatBarrier
...
\begin{flalign}
	y(T=\SI{23,5}{\celsius}) &= 2,21*x - 9,88 \quad | \quad R^2 = 0,6849\\
	...
	\makebox[\textwidth]{
		\resizebox{15cm}{!}{
			\begin{tabular}{c|c|c|c|c}
				\hline
				\textbf{Temperatur in \si{\celsius}} & \textbf{$\ln{\text{k}}$} & \textbf{Stoßfaktor k in} & \textbf{$\frac{1}{\text{T}}$ in \si{\kelvin}}&  \textbf{Reaktionsordnung n in}\\
				\hline 
...
\begin{table}[h!]
	...
	\makebox[\textwidth]{
		\resizebox{7cm}{!}{
			\begin{tabular}{c|c|c}
				\hline
				\textbf{$\ln k_0$}&\textbf{$k_0$} & \textbf{$E_A$ in \si{\kilo \joule \per \mole}} \\
				\hline
